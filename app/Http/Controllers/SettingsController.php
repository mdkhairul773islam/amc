...
class SettingsController extends Controller {
    ...
    public function store(Request $request) {
        ...
        if (!empty($request->file('nav_logo'))) {
            if (Settings::where('meta_key', 'nav_logo')->first()) {
                $data = Settings::where('meta_key', 'nav_logo')->first();
            } else {
                $data = new Settings;
            }
            if (file_exists($data->meta_value)) {
            ...
            $data->meta_key   = 'nav_logo';
            $data->meta_value = uploadFile($request->file('nav_logo'), 'public/uploads/logo');
            $data->save();
        }
